...
export class CoolBearsClient {
  ...
  async reveal(state, map) {
    ...
    const assets = await getAssetV1GpaBuilder(this.umi).whereField('key', Key.AssetV1)
      .whereField('updateAuthority', updateAuthority('Collection', [collection.publicKey]))
      .getDeserialized({ commitment: 'finalized' });
    if (assets.length !== collection.currentSize) throw Error('Collection listing is incomplete; no reveal started');
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      // The final metadata must be publicly retrievable before changing the NFT.
      // Until reveal day this code is unreachable, and private CIDs stay local.
      const uri = item.uri.startsWith('ipfs://') ? `https://ipfs.io/ipfs/${item.uri.slice(7)}` : item.uri;
      const response = await fetch(uri, { signal: AbortSignal.timeout(20000) });
      if (!response.ok) throw Error('Final metadata is not yet publicly available');
      const metadata = await response.json();
      if (metadata.name !== item.name || !Array.isArray(metadata.attributes) || metadata.attributes.length !== 7 || !metadata.image) throw Error('Published reveal metadata does not match');
