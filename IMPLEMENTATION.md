...
RPC recovery: Web3.js and Umi share a serialized HTTP transport with a 350 ms minimum gap. The default public Devnet connection can fail over to the documented OnFinality public Devnet endpoint; each endpoint's full genesis hash is checked before use. HTTP/JSON 429, temporary server failures and timeouts have bounded retries for explicitly allowed read/simulation methods, respect Retry-After and preserve the existing deployment journal. Transaction submission, wallet prompts and unknown RPC methods are never automatically retried by this transport. A custom RPC URL stays on that endpoint. A production/private RPC is still required before mainnet launch. Sources: https://solana.com/docs/references/clusters and https://onfinality.io/en/networks/solana-devnet.
