...
export function createOrderPlanner({ itemsToPlan, validateOrder }) {
...
function checkBlockhash(blockhash, lastValidBlockHeight) {
  ...
  need(valid, 'Supply a valid public blockhash for offline planning');
  need(Number.isSafeInteger(lastValidBlockHeight) && lastValidBlockHeight > 0, 'Supply a positive lastValidBlockHeight');
}

function templateFor(order, item, { blockhash, lastValidBlockHeight }, buyer, umi) {
    ...
    need(unsignedBytes.length <= MAX_TRANSACTION_BYTES, 'One-asset mint exceeds the transaction size limit');
    const requiredSigners = builder.getSigners(umi).map(signer => signer.publicKey);
    need(JSON.stringify(requiredSigners) === JSON.stringify([order.buyer, item.asset]), 'Unexpected signer roles in offline transaction');
    need(Object.values(transaction.signatures).every(signature => !signature.some(Boolean)), 'Offline template contains an unexpected signature');
    return Object.freeze({
      ...
      messageSha256: bytesToHex(sha256(transaction.serializedMessage)),
...
// Structural reconstruction only. This does not authorize an attempt or signer.
function buildOrderItemTemplate(order, index, block) {
  validateOrder(order);
  need(Number.isSafeInteger(index) && index >= 0 && index < order.quantity, 'INVALID_ITEM_INDEX');
  checkBlockhash(block?.blockhash, block?.lastValidBlockHeight);
  const buyer = placeholderSigner(order.buyer);
  return templateFor(order, order.items[index], block, buyer, offlineContext(buyer));
}

function buildOrderTransactions(order, { blockhash, lastValidBlockHeight, retry = false } = {}) {
  need(typeof retry === 'boolean', 'Retry planning must be explicitly boolean');
  ...
  const templates = items.map(item => templateFor(order, item, { blockhash, lastValidBlockHeight }, buyer, umi));
  return Object.freeze({
...
return { buildOrderTransactions, buildOrderItemTemplate };
}
