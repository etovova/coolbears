...
function keys(value, expected) {
  ...
  try { assert.deepEqual(Object.keys(value).sort(), expected.split(' ').sort()); }
...
export function createOrder({ id, cluster, buyer, machine, collection, guard, quantity, available, assets }) {
...
export function itemsToPlan(order, { retry = false } = {}) {
  validateOrder(order);
...
export function readOrder(storage, id) {
...
export function saveOrder(storage, order, expectedRevision = null) {
  validateOrder(order);
