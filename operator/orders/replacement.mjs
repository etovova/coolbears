// Portable structural binding. Only the gateway's retained expiry record authorizes preparation.
...
export function validateReplacementSource(input){
  ...
  need(!order.paused&&order.cluster==='devnet'&&order.items[0].attempts.length===1&&order.items[0].attempts[0].state==='expired'
...
export function replacementCandidate(order,record){
  ...
  need(exact(record,'version kind prior anchor transactionBase64 replacementId')&&record.version===1&&record.kind==='coolbears-buyer-replacement'
    &&record.replacementId===recordId(record)&&order.items[0].attempts.length===1&&order.items[0].attempts[0].state==='expired'
  ...
  need(same(prepared.anchor,record.anchor)&&prepared.candidate.transactionBase64===record.transactionBase64
    ...
    &&record.anchor.sourceSlot>=Math.max(record.prior.proof.slot,record.prior.proof.accountSlot,record.prior.proof.statusSlot)
    &&record.anchor.lastValidBlockHeight>record.prior.proof.blockHeight+80);
  return prepared.candidate;
}
export function replacementFor(input,prior,block,sourceSlot){
  validateReplacementPrior(input,prior);const {anchor,candidate}=preparationFor(input.order,block,sourceSlot);
  const record={version:1,kind:'coolbears-buyer-replacement',prior:structuredClone(prior),anchor,transactionBase64:candidate.transactionBase64};
...
export function replacementReport(input,record,restored=false){
  validateReplacementPrior(input,record?.prior);need(typeof restored==='boolean');
