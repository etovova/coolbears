...
import {verifyBuyerSigningResponse} from './signing.mjs';
import {validateBlockhashAnchor} from './blockhash-anchor.mjs';
import {validateMissingBuyerResponse,recoveredSubmission,responseRecoveryReport} from './response-recovery.mjs';
...
export async function discoverBuyerResponse({input,blockhashAnchor,endpoint,fetchImpl,timeoutMs=12000}){
  ...
  try{
    ...
    const partial=VersionedTransaction.deserialize(Buffer.from(frozen.request.transactionBase64,'base64'));
    for(const row of rows){
      ...
      const signed=verifyBuyerSigningResponse(frozen.order,frozen.claim,frozen.request,{transactionBase64:transaction.transaction[0]});
    ...
    return responseRecoveryReport(frozen,{response:matched.response,result,networkRequests:rpc.requests+extra});
  }catch(error){return responseRecoveryReport(frozen,{networkRequests:(rpc?.requests??0)+extra,
