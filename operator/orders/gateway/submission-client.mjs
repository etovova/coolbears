...
export function createBuyerSubmissionTransport({origin=globalThis.location?.origin,fetchImpl=(...a)=>globalThis.fetch(...a),crypto=globalThis.crypto,timeoutMs=35000}={}){
  ...
  async function call(route,input,costApproval){
    ...
    const body=JSON.stringify({version:1,nonce,...value,...(route==='send'?{costApproval:approval}:route==='review-expiry'?{authorizeExpiryReview:true}:route==='replace'?{authorizeReplacement:true}:{})});need(new TextEncoder().encode(body).length<=65536,'BODY_SIZE');
  ...
  return Object.freeze({send:(input,approval)=>call('send',input,approval),recover:input=>call('recover',input),reviewExpiry:input=>call('review-expiry',input),replace:input=>call('replace',input)});
}
