...
export function createBuyerSubmissionTransport({origin=globalThis.location?.origin,fetchImpl=(...a)=>globalThis.fetch(...a),crypto=globalThis.crypto,timeoutMs=35000}={}){
  ...
  async function call(route,input,costApproval,acknowledgedFeeLamports){
    need(exact(input,route==='recover-response'?'order claim request walletClaim':'order claim request response'),'REQUEST');const value=structuredClone(input);
    if(route==='recover-response')validateMissingBuyerResponse(value);else if(route==='replace')validateReplacementSource(value);else validateBuyerSubmission(value);
    ...
    try{
      ...
      const report=route==='recover-response'?validateResponseRecovery(result.report,value):route==='replace'?validateReplacementResult(result.report,value):route==='review-expiry'?validateBuyerExpiryResult(result.report,value):validateBuyerResult(result.report,value,{recovery:route==='recover'});
  ...
  return Object.freeze({send:(input,approval)=>call('send',input,approval),recover:input=>call('recover',input),recoverResponse:input=>call('recover-response',input),reviewExpiry:input=>call('review-expiry',input),replace:(input,{acknowledgedFeeLamports}={})=>call('replace',input,undefined,acknowledgedFeeLamports)});
}
