...
const parent=await mkdtemp(path.join(tmpdir(),'coolbears-buyer-replacement-fixture-'));
const output=path.resolve('operator/build/buyer-replacement-chromium');await mkdir(output,{recursive:true});
...
try{
  await runtime.start();await launch();const scope=await expired('replacement-linked',true),original=await oldRows(scope);
  ...
  assert.equal(await page.evaluate(()=>code(sender.prepareReplacement())),'EXPLICIT_REPLACEMENT_REQUIRED');assert.equal(fixture.calls.length,calls);
  loseReplacementReply=true;assert.equal(await page.evaluate(()=>code(sender.prepareReplacement({authorizeReplacement:true}))),'SUBMISSION_HTTP');loseReplacementReply=false;
  ...
  assert.equal(prepared.signaturesCreated,0);assert.equal(prepared.transactionsSent,0);assert.deepEqual(await oldRows(scope),original);
  report.cases.push('lost replacement HTTP reply restores the same SQLite record after full browser/server restart without native signature, wallet or send');

  assert.equal(await page.evaluate(async({s,r})=>{try{await store.prepareReplacementSigning(s,r);return 'UNEXPECTED_SUCCESS';}catch(e){return e.message;}},{s:scope,r:prepared}),'EXPLICIT_REPLACEMENT_SIGNING_REQUIRED');
  ...
  assert.notEqual(await page.evaluate(({s,r})=>code(store.prepareReplacementSigning(s,r,{authorizeReplacementSigning:true})),{s:scope,r:prepared}),'UNEXPECTED_SUCCESS');
  const rebound=await replace();assert.deepEqual(rebound.record,prepared.record);assert.equal(fixture.calls.length,afterCommit);
  const partial=await native(scope,rebound);assert.equal(partial.claim.attempt,2);assert.equal(partial.claim.asset,first.signing.claim.asset);
  assert.notEqual(partial.request.transactionBase64,first.signing.request.transactionBase64);assert.deepEqual(await oldRows(scope),original);
  await restart(scope);assert.deepEqual(await page.evaluate(s=>store.readAssetSigning(s),scope),partial);
  ...
  const final=await page.evaluate(s=>store.read(s),scope);assert.deepEqual(final.items[0].attempts.map(a=>a.state),['expired','verified']);
  assert.deepEqual(await oldRows(scope),original);assert.equal((await page.evaluate(s=>store.readBuyerAttempt(s,1),scope)).wallet.response.transactionBase64,first.wallet.response.transactionBase64);
  assert.equal(sends(),sent);assert.equal(await count(),0);
  report.cases.push('pause/resume rejects stale approval; explicit native signing retains prior rows, fresh cost consent gates the second wallet, lost send reply recovers finalized second proof after restart');

  fixture.setGeneration(1);fixture.setMode('normal');const unknown=await signed('replacement-unknown'),unknownCalls=fixture.calls.length;
  assert.equal(await page.evaluate(()=>code(sender.prepareReplacement({authorizeReplacement:true}))),'REPLACEMENT_NOT_READY');assert.equal(fixture.calls.length,unknownCalls);
  assert.equal((await page.evaluate(s=>store.read(s),unknown)).items[0].attempts.length,1);
  report.cases.push('unknown first attempt cannot request a new hash or create a second browser attempt');

  const abort=await expired('replacement-native-abort'),abortRows=await oldRows(abort),abortReport=await replace(),signs=await page.evaluate(()=>assetSignAttempts);
  ...
  assert.deepEqual(await oldRows(abort),abortRows);await page.evaluate(()=>window.writeFailure=null);
  await native(abort,abortReport);assert.equal(await page.evaluate(()=>assetSignAttempts),signs+1);
  report.cases.push('second native claim write abort preserves the expired order and old bytes; no signature occurs before a committed replacement intent');

  const ambiguous=await expired('replacement-native-unknown'),ambiguousRows=await oldRows(ambiguous),ambiguousReport=await replace();
  ...
  report.cases.push('lost second native-signature persistence retains its one-shot claim across restart; original evidence remains readable and no native retry occurs');

  const secondExpiry=await expired('replacement-second-expiry'),secondRows=await oldRows(secondExpiry);
  await native(secondExpiry,await replace());await open(secondExpiry);await spaced();await page.evaluate(()=>approveFixtureCost());await spaced();await page.evaluate(()=>client.signOnly(window.costConsent));
  await page.evaluate(s=>openSender(s),secondExpiry);await spaced();fixture.setMode('expiry-clear');assert.equal((await review()).status,'expired');
  await restart(secondExpiry);const noThird=fixture.calls.length;assert.equal((await review()).status,'already-recorded');
  assert.equal(await page.evaluate(()=>code(sender.prepareReplacement({authorizeReplacement:true}))),'REPLACEMENT_NOT_READY');assert.equal(await failure(),'SEND_NOT_READY');
  assert.deepEqual((await page.evaluate(s=>store.read(s),secondExpiry)).items[0].attempts.map(a=>a.state),['expired','expired']);
  assert.deepEqual(await oldRows(secondExpiry),secondRows);assert.equal(fixture.calls.length,noThird);assert.equal(sends(),sent);assert.equal(await count(),0);
  report.cases.push('second signed unsent expiry is saved separately in IndexedDB and SQLite; restart preserves both histories and denies a third attempt');
