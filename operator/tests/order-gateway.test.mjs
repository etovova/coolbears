...
function harness(extra={}){
  ...
  const storage={async get(k){return structuredClone(k.startsWith('buyer-blockhash:')?f.preparations.get(k):k.startsWith('buyer-cost:')?quotes.get(k):k==='buyer-check-budget:v1'?value:undefined);},async put(k,v){if(fail)throw Error('storage unavailable');if(k.startsWith('buyer-cost:'))quotes.set(k,structuredClone(v));else value=structuredClone(v);},
