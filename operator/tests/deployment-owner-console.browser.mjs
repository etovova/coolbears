...
const { prepareDeploymentSigning } = await import('../deployment/handoff.mjs');
const { prepareDeploymentGroup } = await import('../deployment/group.mjs');
const { startSigningConsole } = await import('../deployment/owner-console/server.mjs');
...
async function open(name, { storageDenied = false, groupCount = 0 } = {}) {
  ...
  const fetchImpl = async (url, init) => {
    ...
    const result = { getGenesisHash: GENESIS_HASHES.devnet,
      getMultipleAccounts: { context: { slot: 510 }, value: groupCount ? insertionAccounts(fixture.manifest) : [{ executable: true }, { executable: true }, { executable: true }, null, null, null, null] },
    ...
    if (groupCount && result?.context) result.context.slot = 600;
    assert.notEqual(result, undefined); return new Response(JSON.stringify({ jsonrpc: '2.0', id: rpc.id, result }));
  };
  if (groupCount) await prepareDeploymentGroup({ directory, count: groupCount, endpoint, fetchImpl });
  else await prepareDeploymentSigning({ directory, stepId: 'collection-create', passphrase, endpoint, fetchImpl });
  ...
  await page.addInitScript(({ address, publicKey, storageDenied }) => {
    ...
    const wallet = { version: '1.0.0', name: 'Fixture Wallet', icon: 'data:image/svg+xml,<svg/>', chains: ['solana:devnet'],
      ...
      features: {
        ...
        'solana:signTransaction': { version: '1.0.0', supportedTransactionVersions: [0], async signTransaction(...inputs) {
          window.__fixtureWalletCalls++;
          if (inputs.some(input => input.chain !== 'solana:devnet')) throw Error('Wrong chain');
          ...
          for (const input of inputs) {
            ...
            outputs.push({ signedTransaction: Uint8Array.from(bytes) });
          }
          return outputs;
        } },
...
try {
  ...
  h = await open('group-lost-ack', { groupCount: 4 }); await ready(h);
  assert.match(await h.page.locator('#progressText').textContent(), /3 из 1431/);
  assert.match(await h.page.locator('#groupInfo').textContent(), /4 операции/);
  await h.page.screenshot({ path: path.join(output, 'group-ready-desktop.png'), fullPage: true });
  await h.page.setViewportSize({ width: 390, height: 844 });
  assert.equal(await h.page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await h.page.screenshot({ path: path.join(output, 'group-ready-mobile-layout.png'), fullPage: true });
  let groupDropped = false; const beforeGroup = signingCalls;
  await h.page.route('**/api/group-signature', async route => {
    const response = await route.fetch(); assert.equal(response.status(), 200);
    if (!groupDropped) { groupDropped = true; return route.abort(); }
    return route.fulfill({ response });
  });
  await h.page.getByRole('button', { name: 'Проверить и подписать группу (4)', exact: true }).click();
  await h.page.getByRole('button', { name: 'Сохранить готовую подпись', exact: true }).waitFor();
  assert.equal(signingCalls - beforeGroup, 4);
  assert.equal(await h.page.evaluate(() => window.__fixtureWalletCalls), 1);
  assert.equal((await h.snapshot()).revision, 12);
  await h.page.reload();
  await h.page.getByRole('status').filter({ hasText: 'Подпись сохранена в журнале' }).waitFor();
  assert.equal(await h.page.getByRole('button', { name: 'Проверить и подписать группу (4)', exact: true }).isDisabled(), true);
  assert.equal(signingCalls - beforeGroup, 4);
  assert.equal(await h.page.evaluate(() => window.__fixtureWalletCalls), 0);
  assert.equal((await h.snapshot()).revision, 12);
  assert.equal(await h.page.locator('#progress').getAttribute('value'), '3');
  assert.equal(h.calls.includes('sendTransaction'), false);
  await h.done('four insertion signatures in one wallet invocation; lost acknowledgment and reload recover atomically without sending');
