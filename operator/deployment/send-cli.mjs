...
import { sendDeploymentStep, resumeDeploymentStep } from './sender.mjs';
...
export async function runDeploymentSenderCli(argv, { env = process.env, fetchImpl,
  ...
  if (!['send-one', 'resume'].includes(mode) || !directory || !stepId || extra.length
    || (mode === 'send-one' ? flag !== '--devnet-send' : flag !== undefined)) {
    write('Usage: send-cli.mjs send-one <bundle> <step> --devnet-send OR resume <bundle> <step>\n');
    return 1;
  }
  try {
    ...
    const run = mode === 'send-one' ? sendDeploymentStep : resumeDeploymentStep;
    const report = await run({ directory, stepId, authorizeDevnetSend: mode === 'send-one', endpoint, fetchImpl: gatewayFetch });
    write(JSON.stringify(report, null, 2) + '\n');
    return ['accepted', 'verified', 'already-recorded'].includes(report.status) ? 0 : 1;
  } catch {
