// Local operator transport for the full closed deployment. Not a public Worker.
// The existing browser/laboratory RPC and its durable ledger are not imported.
...
const PROGRAMS = ['CoREENxT6tW1HoK8ypY1SxRMZTcVPm7R94rH4PZNhX7d',
...
function messageIdentity(bytes) {
...
export async function createScopedDeploymentRpc({ manifest, endpoint, fetchImpl, timeoutMs,
  ...
  need(typeof allowSimulation === 'boolean', 'CONFIGURATION');
  need(Array.isArray(recoverySignatures) && recoverySignatures.length <= 20000
    && recoverySignatures.every(signature), 'CONFIGURATION');
  const recovery = new Set(recoverySignatures); // Trusted local journal input; not proof of finalization.
  const { plan, model } = await buildDeploymentCostModel(manifest);
  // Mainnet is intentionally unavailable at this preparation stage.
  need(plan.cluster === 'devnet', 'CLUSTER');
  const accounts = [...PROGRAMS, plan.roles.collection, plan.roles.reservedAsset, plan.roles.machine, plan.roles.guard];
  const identities = new Set(plan.steps.map(step => messageIdentity(
    Buffer.from(VersionedTransaction.deserialize(Buffer.from(step.transactionBase64, 'base64')).message.serialize()))));
  ...
  function validate(method, params) {
    ...
    switch (method) {
      ...
      case 'getBalance': need(params.length === 2 && first === plan.roles.owner && config(second, 'finalized')); break;
