...
let created = false;
try {
  if (!directory || new Set(flags).size !== flags.length || flags.some(flag => !['--allow-simulation', '--allow-submission'].includes(flag))
    || (flags.includes('--allow-submission') && !flags.includes('--allow-simulation'))) throw Error();
...
const { worker, DeploymentGate } = makeGateway(policy, { allowSubmission: ${allowSubmission} });
export { DeploymentGate };
export default worker;
`, { flag: 'wx', mode: 0o600 });
  console.log(JSON.stringify({ status: 'prepared-offline', messages: policy.messageIdentities.length,
    recoverySignatures: policy.recoverySignatures.length, allowSimulation: policy.allowSimulation, allowSubmission,
