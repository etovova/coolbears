...
Bearer token не освобождают этот claim. Повтор получает HTTP 409. PR34 допускает одну новую signature после серверного доказательства failed.
...
[RETRY.md](RETRY.md). Доказательства `expired` и пакетной очереди пока нет. Даже ошибка до фактического upstream
