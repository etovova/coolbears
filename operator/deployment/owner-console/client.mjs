...
export function createOwnerClient({ api, storage, onChange = () => {} }) {
  ...
  async function save() {
    ...
    checkRequest(record.request); verifySigningResponse(record.request, { transactionBase64: record.transactionBase64 });
    const result = await api('/api/signature', { requestId: view.requestId, transactionBase64: record.transactionBase64 });
  ...
  return {
    async load() {
      ...
      if (record) {
        ...
        if (record.status === 'signed') verifySigningResponse(record.request, { transactionBase64: record.transactionBase64 });
      }
      return this.state();
    },
    ...
    async sign() {
      ...
      try {
        ...
        const request = structuredClone(checked.request); checkRequest(request);
        ...
        await persist({ requestId: view.requestId, request, claimId: checked.claimId, status: 'wallet-pending' });
        need(checked.expiresAt > Date.now(), 'PREFLIGHT_BLOCKED');
        let outputs;
        try { outputs = await selected.features[feature].signTransaction({ account, chain,
          transaction: Uint8Array.from(Buffer.from(request.transactionBase64, 'base64')) }); }
        catch (error) {
          let declined = false;
        ...
        need(Array.isArray(outputs) && outputs.length === 1 && outputs[0]?.signedTransaction instanceof Uint8Array, 'WALLET_RESPONSE');
        ...
        record = { requestId: view.requestId, request, status: 'signed', transactionBase64, accepted: false };
