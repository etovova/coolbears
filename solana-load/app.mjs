...
$('connect').onclick=()=>run(async()=>{
  ...
  const detected=window.phantom?.solana||(window.solana?.isPhantom?window.solana:null);
  if(!detected?.isPhantom)throw Error('Открой эту страницу в браузере Phantom.');
...
try{rpcLabel();$('export').disabled=false;activity('Подключи Phantom, чтобы продолжить.');}catch{activity('Не удалось прочитать настройки браузера.',true);}
render();
