...
def main():
    ...
    assert a.car.name=='description-migration.car'
    proof=json.loads(Path('release/description-correction/preparation.json').read_text())
    assert a.car.stat().st_size==12779698835 and H['file_sha'](a.car)==proof['sourceCarSha256']
    index={}
    ...
    assert index[old_collection][1]==index[old_root][0] and index[old_root][1]==a.car.stat().st_size
    ...
    assert a.car.stat().st_size==proof['carBytes'] and H['file_sha'](a.car)==proof['carSha256']
    print(json.dumps({'status':'STORED_DESCRIPTION_PATCHED_EXACTLY','carSha256':proof['carSha256'],'carBytes':proof['carBytes'],'blocksVerified':len(index),'unchangedNfts':10000,'privateIdentifiersExposed':False}))
