...
const result={schema:2,status:'TUS_PRIVATE_PROBE_NOT_COMPLETED',createStatus:null,patchStatus:null,listed:false,reusedExisting:false,privateEndpoint:true,bytes:DATA.length,idsExposed:false,cidsExposed:false,urlsExposed:false,nftDataUsed:false};
try{
  const existing=await api.api('/v3/files/private?limit=100');
  const existingRows=existing?.data?.files;requireThat(Array.isArray(existingRows),'PRIVATE_LIST_SHAPE_MISMATCH');
  const exact=existingRows.filter(x=>x?.name===NAME&&Number(x?.size||0)===DATA.length);
  requireThat(exact.length<=1,'TUS_PROBE_DUPLICATE');
  if(exact.length===1){
    result.listed=true;result.reusedExisting=true;result.status='TUS_PRIVATE_PROTOCOL_VERIFIED';
  }else{
    ...
    result.patchStatus=patch.status;requireThat(patch.status===204,'TUS_PATCH_HTTP_'+patch.status);
    ...
    requireThat(result.listed,'TUS_PRIVATE_FILE_NOT_LISTED');result.status='TUS_PRIVATE_PROTOCOL_VERIFIED';
  }
