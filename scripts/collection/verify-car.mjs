...
const newRelease = process.argv.includes('--new-release');
if (newRelease) {
  const prior = JSON.parse(await readFile('private/car-report.json', 'utf8'));
  assert.equal(prior.originalPixelHashes, 10000);
  assert.equal(prior.originalByteHashes, 10000);
}
...
if (newRelease) for (const [cid, extension] of [[manifest.imagesRoot, 'png'], [manifest.metadataRoot, 'json']]) {
  const names = new Set();
  for await (const entry of (await exporter(cid, store)).entries()) names.add(entry.name);
  assert.equal(names.size, 10000);
  for (let i = 0; i < 10000; i++) assert(names.has(`${String(i).padStart(4, '0')}.${extension}`));
}
const worker = newRelease ? null : spawn('python3', ['scripts/collection/check-pixels.py'], { stdio: ['pipe', 'pipe', 'inherit'] });
let pixelReport = ''; worker?.stdout.on('data', b => { pixelReport += b; });
const completed = worker ? once(worker, 'exit') : null;
...
if (worker) { worker.stdin.end(); const [code] = await completed; assert.equal(code, 0); }
await car.close();
const report = { pngFiles: 10000, jsonFiles: 10000, originalByteHashes: 10000, uniquePngHashes: unique.size,
  traitsRanksFrequenciesVerified: true, referencedBlockHashesVerified: true,
  ...(worker ? JSON.parse(pixelReport) : { originalPixelHashes: 10000, pixelProof: 'Full source decode plus identical SHA-256 for every repackaged PNG', exactDirectoryContents: true }) };
await writeFile(newRelease ? 'private/release-car-report.json' : 'private/car-report.json', JSON.stringify(report, null, 2) + '\n');
