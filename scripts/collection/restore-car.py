...
p = argparse.ArgumentParser(); p.add_argument('--parts', required=True); p.add_argument('--output', required=True); p.add_argument('--manifest', default='manifest.json')
a = p.parse_args(); source = Path(a.parts); out = Path(a.output)
manifest = json.loads((source / a.manifest).read_text())
assert 1 <= len(manifest['parts']) <= 1000
...
with temp.open('wb') as target:
    for index, part in enumerate(manifest['parts']):
        ...
        size += count
        print(f'Verified part {index + 1}/{len(manifest["parts"])}', flush=True)
    target.flush(); os.fsync(target.fileno())
assert size == manifest['bytes'] and total.hexdigest() == manifest['sha256']
temp.replace(out)
print(json.dumps({'parts': len(manifest['parts']), 'bytes': size, 'sha256': total.hexdigest()}))
