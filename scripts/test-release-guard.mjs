...
test('Missing final branding proof blocks preparation',()=>{const s=clone(prepared);delete s.evidence.finalCollectionBranding;assert.throws(()=>validateLaunch(cfg,p,s,PACKAGE_SHA256));});
for(const key of ['status','packageSha256','appliesToCurrentCandidate','manifestSha256','carSha256','finalContentCommitment','approvedLogoSha256','approvedBannerSha256','unchangedPngFiles','unchangedMetadataFiles','sourceCarExactlyVerified','actualRevealTvmVerified'])test('Incomplete final branding proof '+key,()=>{const s=clone(prepared);s.evidence.finalCollectionBranding[key]=null;assert.throws(()=>validateLaunch(cfg,p,s,PACKAGE_SHA256));});
test('Historical storage rejected even with a matching package hash',()=>{const s=clone(prepared);s.evidence.privateStorage.appliesToCurrentCandidate=false;assert.throws(()=>validateLaunch(cfg,p,s,PACKAGE_SHA256));});
...
for(const key of ['logoVisible','bannerVisible','gifAnimated','attributesAbsent','percentagesAbsent','rankAbsent','collectionDescriptionMatched'])test('Incomplete marketplace proof '+key,()=>{const s=clone(prepared);s.evidence.prerevealGetgemsUi[key]=false;assert.throws(()=>validateLaunch(cfg,p,s,PACKAGE_SHA256));});
