(() => {
  ...
  const tr = {
    en: {
      ...
      mintNow:'MINT NOW', viewGetgems:'VIEW ON GETGEMS', statPrice:'Mint price', statSupply:'Total supply', statRoyalty:'Royalty', statTx:'Per transaction',
      ...
      mintKicker:'PRIMARY MINT', mintTitle:'MINT YOUR<br>COOLBEAR', mintText:'Choose between 1 and 50 NFTs. There is no lifetime wallet limit. Each NFT costs 7 GRAM (TON).',
      mintNoteLine:'NFTs are tradable immediately after mint.', mintedLabel:'MINTED', quantity:'QUANTITY', total:'TOTAL', connectToMint:'CONNECT WALLET TO MINT', mintN:'MINT {n} NFT', mintFoot:'Up to 50 per transaction • no wallet limit',
      ...
      revealLabel:'REVEAL', revealDate:'JANUARY 1', revealFoot:'On reveal day, the same NFT switches to its final CoolBear and its traits. No reroll.',
      ...
      creatorReserve:'The highest-ranked CoolBear by the collection’s rarity score is reserved for the creator and excluded from public mint. Its artwork and traits remain hidden until reveal.',
      top50Title:'TOP 50 RAREST COOLBEARS', top50Text:'Hidden until reveal. Ranked by final rarity score.', top50Stamp:'TOP 50',
      faqTitle:'QUICK<br>ANSWERS.', faq1q:'Can I sell my NFT before reveal?', faq1a:'Yes. The NFT is transferable and tradable immediately after mint. Before reveal, everyone sees the shared GIF.', faq2q:'When do my traits appear?', faq2a:'On January 1, when final metadata is revealed.', faq3q:'Where can I trade?', faq3a:'After Getgems indexes the collection, owners can list and trade there. The official link will be added after deployment.', faq4q:'How is rarity calculated?', faq4a:'Rarity score is based on the real frequency of each final trait across all 10,000 CoolBears.',
      ...
      pageTitle:'CoolBears — 10,000 Bears on GRAM', pageDescription:'CoolBears — 10,000 unique bears. Mint 7 GRAM (TON). Reveal January 1.'
    },
    ru: {
      ...
      mintNow:'ЗАМИНТИТЬ', viewGetgems:'СМОТРЕТЬ НА GETGEMS', statPrice:'Цена минта', statSupply:'Всего', statRoyalty:'Роялти', statTx:'За транзакцию',
      ...
      mintKicker:'ПЕРВИЧНЫЙ МИНТ', mintTitle:'ЗАМИНТЬ СВОЕГО<br>COOLBEAR', mintText:'Выбери от 1 до 50 NFT за одну транзакцию. Общего лимита на кошелёк нет. Цена одного NFT — 7 GRAM (TON).',
      mintNoteLine:'NFT можно перепродавать сразу после минта.', mintedLabel:'ЗАМИНЧЕНО', quantity:'КОЛИЧЕСТВО', total:'ИТОГО', connectToMint:'ПОДКЛЮЧИТЬ КОШЕЛЁК', mintN:'ЗАМИНТИТЬ {n} NFT', mintFoot:'До 50 за транзакцию • без лимита на кошелёк',
      ...
      revealLabel:'РАСКРЫТИЕ', revealDate:'1 ЯНВАРЯ', revealFoot:'В день раскрытия тот же NFT переключится на финального CoolBear и покажет его характеристики. Без повторного выбора.',
      ...
      creatorReserve:'Самый редкий CoolBear по рейтингу коллекции зарезервирован создателю и исключён из публичного минта. Его изображение и атрибуты скрыты до раскрытия.',
      ...
      pageTitle:'CoolBears — 10 000 медведей на GRAM', pageDescription:'CoolBears — 10 000 уникальных медведей. Минт за 7 GRAM (TON). Раскрытие 1 января.'
    },
    zh: {
      ...
      mintNow:'立即铸造', viewGetgems:'在 GETGEMS 查看', statPrice:'铸造价格', statSupply:'总量', statRoyalty:'版税', statTx:'每笔交易',
      ...
      mintKicker:'首次铸造', mintTitle:'铸造你的<br>COOLBEAR', mintText:'每次可选择 1–50 个 NFT。钱包没有总数量限制。每个 NFT 价格为 7 GRAM (TON)。',
      mintNoteLine:'NFT 铸造后即可立即交易。', mintedLabel:'已铸造', quantity:'数量', total:'总计', connectToMint:'连接钱包开始铸造', mintN:'铸造 {n} 个 NFT', mintFoot:'每笔最多 50 个 • 钱包无总量限制',
      beforeReveal:'揭晓之前', revealKicker:'先保持神秘', revealTitle:'你的熊<br>还在隐藏。', revealText:'铸造后，每个 NFT 都显示官方 CoolBears GIF。属性和稀有度保持隐藏，但 NFT 已可出售、转移或赠送。',
      creatorReserve:'按本系列稀有度评分排名第一的 CoolBear 为创作者预留，不参与公开铸造。其图像和属性在揭晓前保持隐藏。', revealLabel:'揭晓', revealDate:'1 月 1 日', revealFoot:'揭晓当天，同一个 NFT 会切换为最终 CoolBear 并显示全部属性，不可重新随机。',
      ...
      faqTitle:'快速<br>解答。', faq1q:'揭晓前可以出售 NFT 吗？', faq1a:'可以。NFT 铸造后即可转移和交易。揭晓前所有人看到同一个 GIF。', faq2q:'什么时候显示属性？', faq2a:'1 月 1 日最终元数据揭晓时。', faq3q:'在哪里交易？', faq3a:'Getgems 完成系列索引后即可在那里上架和交易。正式链接将在部署后添加。', faq4q:'稀有度如何计算？', faq4a:'根据全部 10,000 个 CoolBears 中每个最终属性的真实出现频率计算。',
      ...
      pageTitle:'CoolBears — GRAM 上的 10,000 只酷熊', pageDescription:'CoolBears — 10,000 只独特酷熊。7 GRAM (TON) 铸造。1 月 1 日揭晓。'
    }
  };
  ...
  function clamp() {
    if (!qty || !total) return;
    const max = Number(cfg.maxPerTransaction || 50);
    let value = parseInt(qty.value || '1', 10);
    if (!Number.isFinite(value)) value = 1;
    value = Math.max(1, Math.min(max, value));
    qty.value = value;
    total.textContent = (value * Number(cfg.priceTon || 7)).toLocaleString('en-US');
    renderConnectionState();
  }
  ...
  if (cfg.getgemsUrl && getgems) {
    getgems.href = cfg.getgemsUrl;
    getgems.classList.remove('hidden-link');
  }

  const minted = Number(cfg.mintedCount || 0);
  const supply = Number(cfg.supply || 10000);
  ...
  mintBtn?.addEventListener('click', async () => {
    ...
    if (note) note.textContent = t('contractPending');
  });
